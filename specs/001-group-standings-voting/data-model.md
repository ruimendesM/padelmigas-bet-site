...
| `external_id`              | `integer` NULL            | the ranking sheet's `ID` column — informational only, **not unique**   |
| `display_name`             | `text` NOT NULL           | as published, e.g. `Rodrigo Da Costa`                                  |
| `match_key`                | `text` UNIQUE NOT NULL    | NFC → case-folded → whitespace-collapsed name — **canonical identity** |
...
- The importer aborts if two source rows produce the same `match_key` (research F2 / ADR-007). This
  is now the only guard on identity, so it is load-bearing rather than defensive.
- The importer upserts on `match_key`. It MUST NOT upsert on `external_id`, which can repeat.

> **Amended 2026-08-28.** `external_id` was `integer UNIQUE NOT NULL` and the canonical identity;
> `match_key` was a lookup key. The two swap roles. The live ranking sheet has 784 rows but only 756
> distinct `ID` values — 18 shared by 46 rows describing different people — while all 784 normalised
> names are distinct. The sheet is third-party maintained, so the source cannot be corrected, and a
> unique `external_id` blocks every import outright. `external_id` becomes nullable because a future
> source row may legitimately lack one. Delivered by migration `0006`; see
> [ADR-007](../../docs/adr/ADR-007-player-identity-ranking-sheet.md) § Amendment for the trade-off
> accepted here — chiefly that a rename on the sheet now splits a player in two.
...
### `ranking_snapshots` — verbatim CSV snapshots of the ranking sheet (Risk R3)

| Column       | Type                                        | Notes                                            |
| ------------ | ------------------------------------------- | ------------------------------------------------ |
| `id`         | `uuid` PK                                   |                                                  |
| `csv`        | `text` NOT NULL CHECK (`length(csv) > 0`)   | the sheet exactly as fetched                     |
| `fetched_at` | `timestamptz` NOT NULL                      | when the fetch succeeded                         |
| `row_count`  | `integer` NOT NULL CHECK (`row_count >= 0`) | rows the parser found, for the staleness warning |
| `created_at` | `timestamptz` NOT NULL DEFAULT `now()`      |                                                  |

- Index: `ranking_snapshots (fetched_at desc)` — only the newest snapshot is ever read.
- **Added during implementation (2026-08-27).** Risk R3's mitigation requires storing the raw CSV so
  an unreachable sheet can fall back to the last good copy; this table was missing from the original
  model, which left that mitigation with nowhere to write. The filesystem was rejected: the host is
  serverless and its disk does not survive an invocation.
- Holds real player names and points, which are already published on the public sheet. No other
  personal data (constitution: Privacy).
...
- `players (match_key)` — import path (already UNIQUE; the sole identity key since 2026-08-28).
- `players (external_id)` — non-unique lookup, kept for operator debugging only.
- `ranking_snapshots (fetched_at desc)` — fallback lookup on an unreachable sheet (Risk R3).
...
| Player resolves to exactly one identity               | `core/matching`              | UNIQUE `match_key` (sole guard)          |
